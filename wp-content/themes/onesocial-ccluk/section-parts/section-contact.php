<?php
$ccluk_contact_id            = get_theme_mod( 'ccluk_contact_id', esc_html__('contact', 'onesocial') );
$ccluk_contact_disable       = get_theme_mod( 'ccluk_contact_disable' ) == 1 ?  true : false;
$ccluk_contact_title         = get_theme_mod( 'ccluk_contact_title', esc_html__('Get in touch', 'onesocial' ));
$ccluk_contact_subtitle      = get_theme_mod( 'ccluk_contact_subtitle', esc_html__('Section subtitle', 'onesocial' ));
...
if ( $ccluk_contact_cf7 || $ccluk_contact_text || $ccluk_contact_address_title || $ccluk_contact_phone || $ccluk_contact_email || $ccluk_contact_fax ) {
    $desc = get_theme_mod( 'ccluk_contact_desc' );
    ?>
    <?php if (!$ccluk_contact_disable) : ?>
        <?php if ( ! ccluk_is_selective_refresh() ){ ?>
        <section id="<?php if ($ccluk_contact_id != '') echo $ccluk_contact_id; ?>" <?php do_action('ccluk_section_atts', 'counter'); ?>
            ...
            <div class="container">
                ...
                <div class="row">
                    <?php if ($ccluk_contact_cf7_disable != '1') : ?>
                        ...
                        <?php } else { ?>
                            <div class="contact-form col-sm-6 wow slideInUp">
                                <br>
                                <small>
                                    <i><?php printf(esc_html__('You can install %1$s plugin and go to Customizer &rarr; Section: Contact &rarr; Section Content to show a working contact form here.', 'onesocial'), '<a href="' . esc_url('https://wordpress.org/plugins/contact-form-7/', 'onesocial') . '" target="_blank">Contact Form 7</a>'); ?></i>
                                </small>
                            </div>
                        <?php } ?>
                    <?php endif; ?>
                    ...
                    <div class="col-sm-6 wow slideInUp">
                        <br>
                        <?php if ($ccluk_contact_text != '') echo wp_kses_post($ccluk_contact_text); ?>
                        <br><br>
