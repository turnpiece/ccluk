<?php
$id         = get_theme_mod('ccluk_hero_id', esc_html__('hero', 'ccluk'));
$disable    =  sanitize_text_field(get_theme_mod('ccluk_hero_disable')) == 1 ? true : false;
$fullscreen = sanitize_text_field(get_theme_mod('ccluk_hero_fullscreen'));
$pdtop      = floatval(get_theme_mod('ccluk_hero_pdtop', '10'));
$pdbottom   = floatval(get_theme_mod('ccluk_hero_pdbotom', '10'));
...
<?php if (! $disable && ! empty($images)) : ?>
	<section id="<?php if ($id != '') {
						echo esc_attr($id);
					} ?>" <?php if (! empty($images) && ! $is_parallax) { ?> data-images="<?php echo esc_attr(json_encode($images)); ?>" <?php } ?>
		class="hero-slideshow-wrapper <?php echo ($fullscreen == 1) ? 'hero-slideshow-fullscreen' : 'hero-slideshow-normal'; ?>">

		<?php if (! get_theme_mod('ccluk_hero_disable_preload', false)) { ?>
		...
		?>
			...
			<?php
				break;
			default:
				$hcl1_largetext  = get_theme_mod('ccluk_hcl1_largetext', wp_kses_post('We are <span class="js-rotating">OnePress | One Page | Responsive | Perfection</span>', 'ccluk'));
				$hcl1_smalltext  = get_theme_mod('ccluk_hcl1_smalltext', wp_kses_post('Morbi tempus porta nunc <strong>pharetra quisque</strong> ligula imperdiet posuere<br> vitae felis proin sagittis leo ac tellus blandit sollicitudin quisque vitae placerat.', 'ccluk'));
				$hcl1_btn1_text  = get_theme_mod('ccluk_hcl1_btn1_text', esc_html__('Our Services', 'ccluk'));
				$hcl1_btn1_link  = get_theme_mod('ccluk_hcl1_btn1_link', esc_url(home_url('/')) . esc_html__('#services', 'ccluk'));
				$hcl1_btn2_text  = get_theme_mod('ccluk_hcl1_btn2_text', esc_html__('Get Started', 'ccluk'));
				$hcl1_btn2_link  = get_theme_mod('ccluk_hcl1_btn2_link', esc_url(home_url('/')) . esc_html__('#contact', 'ccluk'));
