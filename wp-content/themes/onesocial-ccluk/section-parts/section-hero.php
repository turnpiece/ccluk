<?php
$id         = get_theme_mod( 'ccluk_hero_id', esc_html__('hero', 'onesocial') );
...
<?php if ( ! $disable && ! empty ( $images ) ) : ?>
	<section  id="<?php if ( $id != '' ){ echo esc_attr( $id ); } ?>" <?php if ( ! empty ( $images) && ! $is_parallax ) { ?> data-images="<?php echo esc_attr( json_encode( $images ) ); ?>"<?php } ?>
		...
		switch( $layout ) {
			...
			break;
			default:
				$hcl1_largetext  = get_theme_mod( 'ccluk_hcl1_largetext', wp_kses_post('We are <span class="js-rotating">OnePress | One Page | Responsive | Perfection</span>', 'onesocial' ));
