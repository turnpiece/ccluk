...
/*-----------------------------------------------------------------------------------*/
/*  CCLUK Customizer Controls
/*-----------------------------------------------------------------------------------*/

class CCLUK_Misc_Control extends WP_Customize_Control {
 ...
 * @return string
 */
function ccluk_sanitize_css($string) {
...
function ccluk_sanitize_color_alpha( $color ){
 ...
 */
function ccluk_sanitize_repeatable_data_field( $input , $setting ){
            ...
            if ( isset( $fields[ $id ] ) ){
                switch( strtolower( $fields[ $id ]['type'] ) ) {
                    ...
                    case 'color':
                        ...
                        break;
                    case 'coloralpha':
                        $data[ $i ][ $id ] = ccluk_sanitize_color_alpha( $value );
                        break;
                    case 'checkbox':
                        $data[ $i ][ $id ] =  ccluk_sanitize_checkbox( $value );
                        break;
                    case 'select':
                        $data[ $i ][ $id ] = '';
...
class CCLUK_Editor_Custom_Control extends WP_Customize_Control
{
    /**
 ...
 */
class CCLUK_Alpha_Color_Control extends WP_Customize_Control {
...
class Onepress_Customize_Repeatable_Control extends WP_Customize_Control {
    ...
    public function __construct( $manager, $id, $args = array() )
    {
        ...
        if ( ! isset( $args['allow_unlimited'] ) || $args['allow_unlimited'] != false ) {
            $this->max_item =  apply_filters( 'ccluk_reepeatable_max_item', absint( $args['max_item'] ) );
        }  else {
                                    ...
                                    #>
                                        ...
                                        <# } else if ( field.type == 'media' ) { #>
                                            ...
                                            <div class="actions">
                                                ...
                                                <button class="button upload-button" data-media="{{field.media}}" data-add-txt="<?php esc_attr_e( 'Add', 'onesocial' ); ?>" data-change-txt="<?php esc_attr_e( 'Change', 'onesocial' ); ?>" type="button"><# if ( ! field.value.url  ){ #> <?php _e( 'Add', 'onesocial' ); ?> <# } else { #> <?php _e( 'Change', 'onesocial' ); ?> <# } #> </button>
...
function onepres_customizer_control_scripts(){
    ...
    wp_enqueue_script( 'ccluk-customizer', get_stylesheet_directory_uri() . '/assets/js/customizer.js', array( 'customize-controls', 'wp-color-picker' ) );
    wp_enqueue_style( 'ccluk-customizer',  get_stylesheet_directory_uri() . '/assets/css/customizer.css' );

}

add_action( 'customize_controls_enqueue_scripts', 'onepres_customizer_control_scripts', 99 );
add_action( 'customize_controls_enqueue_scripts', array( 'CCLUK_Editor_Scripts', 'enqueue' ), 95 );
