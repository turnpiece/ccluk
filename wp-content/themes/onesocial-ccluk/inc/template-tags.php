...
if (! function_exists('ccluk_posted_on')) {
    ...
    function ccluk_posted_on()
    {
        ...
        $posted_on = sprintf(
            esc_html_x('Posted on %s', 'post date', 'ccluk'),
            '<a href="' . esc_url(get_permalink()) . '" rel="bookmark">' . $time_string . '</a>'
        );

        $byline = sprintf(
            esc_html_x('by %s', 'post author', 'ccluk'),
...
if (! function_exists('ccluk_entry_footer')) {
    ...
    function ccluk_entry_footer()
    {
        ...
        if ('post' === get_post_type()) {
            ...
            $categories_list = get_the_category_list(esc_html__(', ', 'ccluk'));
            if ($categories_list && ccluk_categorized_blog()) {
                printf('<span class="cat-links">' . esc_html__('Posted in %1$s', 'ccluk') . '</span>', $categories_list); // WPCS: XSS OK.
            }

            /* translators: used between list items, there is a space after the comma */
            $tags_list = get_the_tag_list('', esc_html__(', ', 'ccluk'));
            if ($tags_list) {
                printf('<span class="tags-links">' . esc_html__('Tagged %1$s', 'ccluk') . '</span>', $tags_list); // WPCS: XSS OK.
            }
        }

        if (!is_single() && !post_password_required() && (comments_open() || get_comments_number())) {
            echo '<span class="comments-link">';
            comments_popup_link(esc_html__('Leave a comment', 'ccluk'), esc_html__('1 Comment', 'ccluk'), esc_html__('% Comments', 'ccluk'));
            echo '</span>';
        }
    }
}
                        ...
                        <div class="comment-wrapper">
                            ...
                            <header class="comment-meta comment-author vcard">
                                ...
                                printf(
                                    ...
                                    sprintf(__('%1$s', 'ccluk'), get_comment_date())
                                );
                                comment_reply_link(array_merge($args, array('reply_text' => __('Reply', 'ccluk'), 'after' => '', 'depth' => $depth, 'max_depth' => $args['max_depth'])));
                                edit_comment_link(__('Edit', 'ccluk'), '<span class="edit-link">', '</span>');
                                ?>
                            </header><!-- .comment-meta -->
...
if (! function_exists('ccluk_footer_site_info')) {
    ...
    {
            ?>
            <?php printf(esc_html__('Copyright %1$s %2$s %3$s', 'ccluk'), '&copy;', esc_attr(date('Y')), esc_attr(get_bloginfo())); ?>
            <span class="sep"> &ndash; </span>
            <?php printf(esc_html__('%1$s theme by %2$s', 'ccluk'), '<a href="' . esc_url('https://www.famethemes.com/themes/onepress', 'ccluk') . '">CCLUK</a>', 'FameThemes'); ?>
        <?php
    }
}
