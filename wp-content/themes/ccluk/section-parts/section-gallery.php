<?php
$id       = get_theme_mod('ccluk_gallery_id', esc_html__('gallery', 'ccluk'));
$disable  = get_theme_mod('ccluk_gallery_disable', 1) ==  1 ? true : false;
$title    = get_theme_mod('ccluk_gallery_title', esc_html__('Gallery', 'ccluk'));
$subtitle = get_theme_mod('ccluk_gallery_subtitle', esc_html__('Section subtitle', 'ccluk'));
...
<?php if (! $disable) { ?>
    <?php if (! ccluk_is_selective_refresh()) { ?>
        ...
        <div class="g-layout-<?php echo esc_attr($layout); ?> <?php echo esc_attr(apply_filters('ccluk_section_container_class', 'container', 'gallery')); ?>">
            ...
            $readmore_text = get_theme_mod('ccluk_g_readmore_text', esc_html__('View More', 'ccluk'));
            if ($readmore_link) {
            ?>
