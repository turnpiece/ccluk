...
;(function ($, window, document, undefined) {
    ...
    Embeds.prototype.embed = function (html, url) {
        ...
        if (!html) {
            alert( sap_loading_string.incorrect_url_format );
            return false;
        } else {
...
;(function ($, window, document, Util, undefined) {
    ...
    Images.prototype.uploadAdd = function (e, data) {
        ...
        /* Allow to add text after image upload: BUDDYBOSS Fix */
        /* Check if p tag exist, if not then add one */
        if( $place.prev('p').length <= 0 ){
            $place.parents('.sap-editable-area').prepend('<p class=""><br></p>');
        }
