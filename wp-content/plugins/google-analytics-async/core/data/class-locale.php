...
class Locale {
	...
	public static function onboarding() {
		return array(
			...
			'desc'   => array(
				...
				'welcome_single'         => sprintf( __( '%s, welcome to the hottest Google Analytics plugin for WordPress. Let\'s get started by connecting your Google Analytics account to feed your analytics data. If you only want to enable tracking without reports, you can paste your Measurement ID via the link blow.', 'ga_trans' ), General::get_user_name() ),
	...
	public static function dashboard() {
		return array(
			...
			'notice' => array(
				...
				'ga4_not_setup'         => sprintf(
				// translators: %s Link to configure GA4.
					__( 'Google is dropping Universal Analytics Property soon. To ensure that all analytics features continue working without interruptions, please <a href="%s">configure the Analytics 4 Property</a> as soon as possible.', 'ga_trans' ),
					Helper::settings_url( 'account', is_network_admin() )
				),
			),
		);
	}
	...
	public static function welcome() {
		return array(
			'title'  => array(
				'new_welcome_title' => __( 'Google is dropping UA soon', 'ga_trans' ),
			),
			'desc'   => array(
				'new_welcome_desc' => __( 'Google has announced that it will be dropping Universal Analytics in July 2023. We recommend setting up a Google Analytics 4 (GA4) stream for your website from our new GA4 settings section to continue tracking visits and receiving Google Analytics statistics without interruptions.', 'ga_trans' ),
			),
			'button' => array(
				'setup_ga4' => __( 'Setup Analytics 4', 'ga_trans' ),
			),
		);
	}
