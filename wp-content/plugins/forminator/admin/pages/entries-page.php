...
class Forminator_Entries_Page extends Forminator_Admin_Page {
	...
	public function get_form_types() {
		...
		foreach ( $this->modules as $module ) {
			...
			$form_types[ $model->getPostType() ] = $name;
		}
	...
	private function prepare_entries_page() {
		...
		if ( ! $this->form_model instanceof Forminator_Base_Form_Model ) {
			...
			switch ( $this->get_current_form_type() ) {
				case Forminator_Custom_Form_Model::model()->getPostType():
					$entries_renderer = new Forminator_CForm_Renderer_Entries( 'custom-form/entries' );
					break;
				case Forminator_Poll_Form_Model::model()->getPostType():
					$entries_renderer = new Forminator_Poll_Renderer_Entries( 'poll/entries' );
					break;
				case Forminator_Quiz_Form_Model::model()->getPostType():
	...
	public function render_form_switcher() {
		...
		} elseif ( $this->get_current_form_type() === Forminator_Quiz_Form_Model::model()->getPostType() ) {
	...
	public function get_forms() {
		...
		switch ( $form_type ) {
			case 'forminator_forms':
				//TODO: lazy load this
				$forms = Forminator_Custom_Form_Model::model()->getModels( 99 );
				break;
			case 'forminator_polls':
				$forms = Forminator_Poll_Form_Model::model()->getModels( 99 );
				break;
			case 'forminator_quizzes':
				$forms = Forminator_Quiz_Form_Model::model()->getModels( 99 );
				break;
			default:
				$forms = array();
				break;
