...
class Forminator_Addon_Mailchimp_Wp_Api {
	...
	private function _request( $verb = 'GET', $path, $args = array() ) {
		...
		if ( is_wp_error( $res ) || ! $res ) {
			...
			throw new Forminator_Addon_Mailchimp_Wp_Api_Exception( __( 'Failed to processing request, make sure API KEY is correct and your server has internet connection.', Forminator::DOMAIN ) );
		}
		...
		if ( 'DELETE' !== $verb ) {
			...
			if ( empty( $body ) ) {
				forminator_addon_maybe_log( __METHOD__, $res );
				throw new Forminator_Addon_Mailchimp_Wp_Api_Exception( __( 'Failed to processing request, make sure API KEY is correct and your server has internet connection.',
				                                                           Forminator::DOMAIN ) );
			}
		}
