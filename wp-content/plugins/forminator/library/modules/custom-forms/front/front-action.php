...
class Forminator_CForm_Front_Action extends Forminator_Front_Action {
				...
				if ( $can_submit ) {
					if ( ! empty( $field_data_array ) && empty( $submit_errors ) ) {
						...
						if ( $entry->save() ) {
							...
							if ( isset( $setting['redirect'] ) && $setting['redirect'] ) {
					...
					if ( ! empty( $submit_errors ) ) {
						$response = array(
							'message' => $this->get_invalid_form_message( $setting ),
							'success' => false,
	 ...
	 * @return mixed
	 */
	public function get_invalid_form_message( $setting ) {
		if ( isset( $setting['use-custom-invalid-form'] ) && $setting['use-custom-invalid-form'] ) {
			if ( isset( $setting['custom-invalid-form-message'] ) && ! empty( $setting['custom-invalid-form-message'] ) ) {
				return $setting['custom-invalid-form-message'];
			}
		}

		return __( "Error: Your form is not valid, please fix the errors!", Forminator::DOMAIN );
	}
