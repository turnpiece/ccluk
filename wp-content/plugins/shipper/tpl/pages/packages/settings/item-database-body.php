...
<div class="sui-form-field">
			...
			<div data-panes>
				...
				<div class="sui-tab-boxed <?php echo ! $shell_active ? 'active' : ''; ?>">
					...
					<select name="database-export-rows">
						<?php foreach ( range( 0, 500, 50 ) as $rows ) { ?>
							<?php $rows = 0 === $rows ? 10 : $rows; ?>
	...
	<?php else: ?>
		...
		<span class="sui-description">
		<?php _e( 'Choose the query limit to build SQL scripts. We recommend using a lower query limit on the budget hosts. A higher limit size will speed up the build time, but it will use more memory.', 'shipper' ); ?>
		</span>
		<input type="radio"
		       name="database-use-binary"
		       value="0"
