...
class Shipper_Controller_Override_Package_Advanced
	...
	public function get_scope() {
		return Shipper_Model_Stored_Package::KEY_EXCLUSIONS_XX;
	}
	...
	public function apply_overrides() {
		...
		if ( in_array( 'post-revisions', $exclusions, true ) ) {
			add_filter(
				...
				array( $this, 'exclude_post_revisions' ), 10, 3
			);
		}
		...
		if ( in_array( 'spam-comments', $exclusions, true ) ) {
			add_filter(
				'shipper_export_table_include_row',
				array( $this, 'exclude_spam_comments' ), 10, 3
			);
		}
		...
		if ( in_array( 'inactive-themes', $exclusions, true ) ) {
			add_filter(
				...
				array( $this, 'exclude_inactive_theme_file' ), 10, 2
			);
		}
		...
		if ( in_array( 'inactive-plugins', $exclusions, true ) ) {
			add_filter(
				'shipper_path_include_file',
				array( $this, 'exclude_inactive_plugin_file' ), 10, 2
			);
		}
	}
	...
	public function exclude_inactive_plugin_file( $include, $path ) {
		...
		if ( ! (bool) stristr( $path, $plugins_path ) ) { return $include; }
	...
	public function exclude_inactive_theme_file( $include, $path ) {
		...
		if ( ! (bool) stristr( $path, $themes_path ) ) { return $include; }
	...
	public function exclude_post_revisions( $include, $row, $table ) {
		...
		$is_post_row = shipper_array_keys_exist( array(
			'ID',
			'post_author',
			'post_status',
			'post_title',
			'post_type',
			'post_content',
		), $row ) && strrpos( $table, 'posts' );
		if ( ! $is_post_row ) { return $include; }
